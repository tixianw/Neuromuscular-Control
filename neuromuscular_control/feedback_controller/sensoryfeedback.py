import numpy as np
from neuromuscular_control.utils import _aver, _aver_kernel
...
class SensoryFeedback:
    ...
    def feedback(self, time, system):
        ...
        error_feedback = (
            self.sin_alpha
        )  # mag_cos # mag_cos - mag_sin # 0.5 * (np.sqrt(3)*mag_cos - mag_sin) #
        ...
        if self.s0 > self.s[10]:
            ## longitudinal
            self.ctrl_mag[0, idx_top] = (
                -1 / (1 + np.exp(-steep * (self.s[idx_top] - (self.s0 - shift * 0))))
                + 1
            )
            self.ctrl_mag[1, idx_bottom] = (
                -1
                / (
                    1
                    + np.exp(
                        -steep * (self.s[idx_bottom] - (self.s0 - shift * sigma * 0))
                    )
                )
                + 1
            )
            ...
            self.ctrl_mag[0, idx_top] = 1.0
            self.ctrl_mag[1, idx_bottom] = 1.0
