...
class NeuralControlMatsuoka:
    def __init__(self, env, callback_list: dict, step_skip: int, ramp_up_time=0.0):
        ...
        if env.flag_shooting:
            ...
            V0 = np.vstack(
                [
                    np.ones([1, self.n_elem + 1])
                    * ((V_t1 - V_t0) / L * (self.s) + V_t0),
                    np.ones([1, self.n_elem + 1])
                    * ((V_b1 - V_b0) / L * (self.s) + V_b0),
    ...
    def backstepping(self, u):
        ...
        self.gamma = 3
        self.I = (
            self.gamma * (self.u_to_V(u) - self.neuron.V)
            + self.neuron.V
            + self.neuron.adaptation_weight * relu(self.neuron.V)
            - self.neuron.lmd
            * self.neuron.lmd
            * _diff_kernel(_diff(self.u_to_V(u)) / self.neuron.ds)
            / self.neuron.ds
        )
