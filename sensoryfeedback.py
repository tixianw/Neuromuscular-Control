...
class SensoryFeedback:
	...
	def feedback(self, time, system):
		...
		if self.s0 > self.s[10]:
			## longitudinal
			self.ctrl_mag[0,idx_top] = (-1 / (1 + np.exp(-steep * (self.s[idx_top] - (self.s0-shift*0)))) + 1)
			self.ctrl_mag[1,idx_bottom] = (-1 / (1 + np.exp(-steep * (self.s[idx_bottom] - (self.s0-shift*sigma*0)))) + 1)
			## transverse
