...
class DragForce(NoForces):
    ...
    def apply_torques(self, system, time: np.float = 0.0):
        ...
        Sa = Pa * np.pi
        self.velocity_material_frame = _lab_to_material(
            system.director_collection, _aver(system.velocity_collection)
        )
        self.force_material_frame = -self.velocity_material_frame * abs(
            self.velocity_material_frame
        )
        self.force_material_frame[-1, :] *= Sa * self.c_tan
        self.force_material_frame[1, :] *= Pa * self.c_per
        self.drag_force = _aver_kernel(
            _material_to_lab(system.director_collection, self.force_material_frame)
        )
...
class ArmEnvironment:
    def __init__(self, final_time, flags, time_step=1.0e-5, recording_fps=100):
        ...
        self.total_steps = (
            int(final_time * 1e5) + 1
        )  # int(self.final_time/self.time_step) #
    ...
    def set_rod(self):
        ...
        radius_base = (
            base_length / 20
        )  # 0.012 # 0.01 # 0.02  # radius of the arm at the base
        radius_tip = (
            radius_base / 10
        )  # 0.0012 # 0.004 # 0.004 # 0.0004  # radius of the arm at the tip
        ...
        if self.flag_shooting:
            ...
            # if direction[0]:
            init_pos = np.vstack(
                [np.linspace(0, base_length, n_elem + 1), np.zeros([2, n_elem + 1])]
            )
            # elif direction[1]:
    ...
    def add_muscles(self):
        ...
        n_max = np.array([1.0, 1, -2]) * 1.0  # 2. # 1.5 # 4
        n_muscle = len(n_max)  # number of muscle fiber
        max_force = (
            self.shearable_rod.radius / self.shearable_rod.radius[0]
        ) ** 2 * n_max[:, None]
        passive_ratio = 1
