...
class Environment(ArmEnvironment):
    ...
    def set_obstacle(self):
        ...
        pos_obs = [
            np.array([0.065, 0.05, 0]),
            np.array([0.14, 0.05, 0]),
        ]  # [np.array([0.1,0.05,0])] # [np.array([0.,0.03,0]), np.array([0.08,0.03,0])] # [np.hstack([target_exp+0.01,0])] #
        ...
        for o in range(N_obs):
            ...
            self.simulator.connect(
                self.shearable_rod, name_obstacle["obstacle" + str(o)]
            ).using(ExternalContact, k=1e2, nu=1.0)
