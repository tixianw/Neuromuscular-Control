...
def longitudinal_muscle_function(
    magnitude_force_mean,
    magnitude_force,
    r_LM,
    max_force,  # radius_ratio, radius,
    director_collection,
    tangents,
    rest_lengths,
    dilatation,
...
@njit(cache=True)
def force_length_curve_poly(
    stretch, f_l_coefficients=np.array([-57.6 / 39.2, 176 / 39.2, -80 / 39.2])
):  # np.array([-6.44, 18.01, -13.64, 3.06])):
...
class ContinuousActuation:
    def __init__(
        self, n_elements: int, n_muscle, max_force, radius_ratio, passive_ratio
    ):
    ...
    def sigma_to_stretch(self, sigma):
        array = _aver_kernel(sigma[-1, :]) + 1.0
        return array
    ...
    def calculate_force_length_weight(self, system):
        ...
        self.nu_muscle = self.muscle_stretch(
            ...
            off_center_displacement=_aver_kernel(
                self.muscle_radius_ratio * system.radius
            ),
        )
    ...
    def __call__(self, system):
        ...
        longitudinal_muscle_function(
            ...
            _aver(self.magnitude)
            * self.max_force
            * self.passive_ratio
            * _aver(self.weight),
            _aver_kernel(self.muscle_radius_ratio * system.radius),
            self.max_force,
            ...
            system.dilatation,
            system.sigma,
            system.kappa[0, :] / system.voronoi_dilatation,
            system.shear_matrix,
            system.bend_matrix,
            # self.internal_forces_mean,
            self.internal_forces,
            self.internal_couples,
            self.external_forces,
            self.external_couples,
            self.muscle_bend_couple,
            self.muscle_shear_couple,
        )
