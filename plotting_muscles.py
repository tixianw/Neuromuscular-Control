...
def plot_arm():
    ...
    i = -1
    ax0.scatter(position[i,0,:],position[i,1,:], s=r, marker='o', alpha=1,zorder=2)
...
if video == 0:
    ...
    min_var = -L/2
    idx = -1
    min_var_x = min(np.amin(position[0,0,:])*1.1, np.amin(position[-1,0,:])*1.1, min_var*1.01)
    max_var_x = max(np.amax(position[-1,0,:])*1.1, np.amax(position[0,0,:])*1.1, max_var*1.01)
...
## V and I from cable equation
    ...
    min_var = -L/2 
    idx = -1
    min_var_x = -.8*L # min(np.amin(position[0,0,:])*1.1, np.amin(position[-1,0,:])*1.1, min_var*1.01)
    max_var_x = 1.1*L # max(np.amax(position[-1,0,:])*1.1, np.amax(position[0,0,:])*1.1, max_var*1.01)
    min_var_y = -.8*L # min(np.amin(position[-1,1,:])*1.1, min_var*1.01)
