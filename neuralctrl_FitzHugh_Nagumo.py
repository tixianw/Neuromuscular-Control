...
class Cable:
	...
	def dynamics_cable(self, V, I):
		...
		inhibition[-1] *= 0
		dVdt = self.lmd*self.lmd * Vss + V - V*V*V/3 + self.V_rest + I - self.V_adapt - self.inhibition_weight * relu(inhibition)
		### Boundary conditions
	...
	def dynamics_adapt(self, V_adapt, I=None):
		dVdt_adapt = -0.8 * V_adapt + self.adaptation_weight * self.V + 0.7 # relu(self.V)
		dVdt_adapt[-1] *= 0
...
class NeuralCtrl:
	def __init__(self, env, callback_list: dict, step_skip: int, ramp_up_time=0.0):
		...
		print('tau',tau,'lmd',lmd0,'tauA',tau_adapt,'inhibit',inhibition_weight,'adapt',adaptation_weight,'V',[V_t0, V_b0])
